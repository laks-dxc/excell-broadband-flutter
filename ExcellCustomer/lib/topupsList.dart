...
class _TopupsListState extends State<TopupsList> {
  ...
  Widget build(BuildContext context) {
    ...
    return topupsLoaded == false
        ? Container(
            child: Center(child: SpinKitCircle(size: 35, color: Colors.grey)))
        : Container(
            ...
            child: Column(
              ...
              children: [
                  ...
                  child: ListView(
                    children: List.generate(this.topups.length, (index) {
                      return Padding(
                        ...
                        child: InkWell(
                            child: tileItem(
                                this.topups[index]["pkg"],
                                Utils.showAsMoney(this
                                    .topups[index]["baseprice"]
                                    .toString())),
                            onTap: () {
                              setState(() {
                                selectedTopupName = this.topups[index]["pkg"];
                                paymentstring =
                                    this.topups[index]["paymentstring"];
                                taxAmount =
                                    (double.parse(this.topups[index]["tax"]) *
                                            double.parse(this
                                                .topups[index]["baseprice"]
                                                .toString()) *
                                            0.01)
                                        .toString();
                                taxLabel =
                                    "GST (" + topups[index]["tax"] + " % )";
                                basePrice =
                                    this.topups[index]["baseprice"].toString();
                                totalPrice =
                                    this.topups[index]["price"].toString();
                              });
                            }),
                      );
                    }),
                ...
                selectedTopupName == null
                    ? disabledButton()
                    : enabledButton(context)
              ],
            ),
          );
  }
