            ...
            case FooterState.ValidCredentialsEntered:
              {
                ...
                Customer.authenticateWithoutOTP(
                        customerIdController.text, mobileNoController.text)
                    .then((authResponse) async {
                  if (authResponse["status"] == 200) {
                    Customer.paymentDueWithToken(
                            authResponse["token"], customerIdController.text)
                        .then((paymentDueResponse) {
                      setState(() {
                        activeContainerBody = "QuickPay";
                        amountDue =
                            Utils.showAsMoney(paymentDueResponse["amount"]);
                        doubleAmoutDue =
                            double.parse(paymentDueResponse["amount"]);
